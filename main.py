...
def run_unpaper(input_folder, output_folder):
    unpaper_command = [
        "unpaper",
        "--layout", "double",  # Adjust layout based on your document (single/double)
        "--input-pages", "1",   # Number of pages per input image (adjust if needed)
        "--output-pages", "2",  # Number of pages per output image
        "--no-blackfilter",  # Keep this for text preservation
        str(Path(input_folder) / "page_%d.ppm"),
        str(Path(output_folder) / "output%03d.ppm")
    ]
